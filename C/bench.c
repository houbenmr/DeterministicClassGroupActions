...
extern uint64_t *fp_add_counter;
extern uint64_t *fp_sub_counter;
...
int main()
{
	/* Initializing base curve */
	/* !! This only works for p512 */

	const public_key base = {fp_0, {fp_28, fp_1}, {fp_m28, fp_1}};
    ...
    uint64_t *cycless = calloc(its, sizeof(uint64_t)),
	     ...
	     *addss = calloc(its, sizeof(uint64_t)),
	     *subss = calloc(its, sizeof(uint64_t)),
	...
	fp_add_counter = &addss[i];
	fp_sub_counter = &subss[i];
    ...
    printf("median additions:       %8" PRIu64 "\n", median(addss));
    printf("mean additions:         %10.1lf\n", mean(addss));

    printf("median subtractions:    %8" PRIu64 "\n", median(subss));
    printf("mean subtractions:      %10.1lf\n", mean(subss));
